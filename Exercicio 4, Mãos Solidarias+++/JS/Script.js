...
// ACESSIBILIDADE E TEMAS (NOVAS FUNÇÕES)
function applyTheme(theme) {
    const body = document.body;
    // Remove todas as classes de tema
    body.classList.remove('dark-mode', 'high-contrast');
    
    // Aplica a classe correspondente se não for o tema padrão
    if (theme === 'dark-mode') {
...
// Mobile e Dropdown & Inicialização
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Inicialização do Tema (Acessibilidade)
    const themeSelect = document.getElementById('theme-select');
    const savedTheme = localStorage.getItem('user-theme') || 'default'; // Pega o tema salvo ou o padrão
    
    // Aplica o tema salvo
    applyTheme(savedTheme);
    
    // Define a opção correta no seletor e adiciona o listener
    if (themeSelect) {
